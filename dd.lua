...
do
	...
	do
		function Library:NewPicker(name, default, parent, count, flag, callback)
			...
			local function update()
				local real_pos = user_input:GetMouseLocation()
			...
			Library:Connection(user_input.InputChanged, function(input)
			...
			Library:Connection(user_input.InputBegan, function(Input)
	...
	do
		...
		function Library:New(Properties)
			...
			--
			local ScreenGui = Instance.new('ScreenGui', core_gui)
			...
			Logo.Name = "Logo"
			Logo.Image = "http://www.roblox.com/asset/?id=73923308560884"
			Logo.ScaleType = Enum.ScaleType.Fit
			Logo.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			Logo.BackgroundTransparency = 1
			Logo.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Logo.BorderSizePixel = 0
			Logo.Position = UDim2.fromOffset(-10, -47)
			...
			MakeDraggable(Outline) -- or whatever your main window frame is
			...
			Library:Connection(user_input.InputBegan, function(Inp)
    ...
    Accent.Position = UDim2.new(0, 5, 0, 6.5)
    ...
    ------------------------------------------------------------
    -- LEFT / RIGHT SCROLLING COLUMNS (IMPORTANT)
    ------------------------------------------------------------
    ...
    function Page:Turn(state)
        ...
        Title.TextColor3 = state and Color3.fromRGB(200,200,200) or Color3.fromRGB(120,120,120)
        Icon.ImageColor3 = Title.TextColor3
    end
...
------------------------------------------------------------
-- SECTION CREATION (NO OVERLAP, SCROLL SAFE)
------------------------------------------------------------
function Pages:Section(Properties)
    ...
    return setmetatable(Section, Library.Sections)
		...
		function Sections:Slider(Properties)
                ...
                SliderValue.Position = UDim2.new(1, -200, 0, 0)
                ...
                local function Slide(Position)
                    local sizeX = (Position.X - NewSlider.AbsolutePosition.X) / NewSlider.AbsoluteSize.X
                ...
                Library:Connection(NewSlider.InputBegan, InputBegan)
                Library:Connection(NewSlider.InputEnded, InputEnded)
                Library:Connection(Fill.InputBegan, InputBegan)
                Library:Connection(Fill.InputEnded, InputEnded)
                Library:Connection(user_input.InputChanged, InputChanged)
		...
		function Sections:Keybind(Properties)
			...
			KeyText.Position = UDim2.new(1, -190, 0, 0)
			...
			--
			Library:Connection(user_input.InputBegan, function(inp, gpe)
				if (gpe) then return end;
			...
			--
			Library:Connection(user_input.InputEnded, function(inp, gpe)
				if gpe then return end;
	...
	function Sections:Textbox(Properties)
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "Synthetic");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"Synthetic" do
						Watermark.AnimateText = string.sub("Synthetic", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;

					for i = #"Synthetic" - 1, 1, -1 do
						Watermark.AnimateText = string.sub("Synthetic", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;
